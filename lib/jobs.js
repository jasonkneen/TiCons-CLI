...
exports.run = function run(spec, cfg, callback) {
	...
	fs.exists(cfg.input, function (exists) {
		...
		// nine or fill
		if (!cfg.crop || (cfg.nine && (spec.ninePath || spec.alsoNine))) {
		...
		if (spec.type === 'icon' && spec.alpha && cfg.radius > 0 && _.size(_.difference(spec.platforms, ['ios', 'ipad', 'iphone', 'apple-watch'])) > 0) {
