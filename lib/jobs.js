...
exports.run = function run(spec, cfg, callback) {
  ...
  if (cfg.nine && spec.ninePath) {
    ...
    if (!cfg.inputWidth || !cfg.inputHeight) {
      return callback('missing required argument `inputWidth` and `inputHeight` for 9-patch to work');
    }

    // resize fitting dimensions
    ...
    var width, height;
    var ratio = cfg.inputWidth / cfg.inputHeight;

    if (ratio > spec.width / spec.height) {
      width = spec.width;
      height = spec.width / ratio;
    } else {
      width = spec.height * ratio;
      height = spec.height;
    }
  ...
  config(opts, function(err, cfg) {
    ...
    _.each(specs, function(spec, name) {
      ...
      if (!_.some(spec.platforms, function(platform) {
        ...
        return !!cfg[platform] || ((platform === 'ipad' || platform === 'iphone') && !! cfg.ios);
