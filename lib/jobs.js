...
exports.run = function run(spec, cfg, callback) {
  ...
  fs.exists(cfg.input, function(exists) {
    ...
    if ((spec.type === 'icon' || spec.type === 'icon-foreground') && !flatten && cfg.radius > 0 && _.size(_.difference(spec.platforms, ['ios', 'ipad', 'iphone', 'apple-watch'])) > 0) {
...
exports.getSpecs = function getSpecs(opts, callback) {
  ...
  config(opts, function(err, cfg) {
    ...
    // handle DefaultIcon
    if (cfg.type === 'icon' || cfg.type === "icon-foreground") {
