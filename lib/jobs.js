...
function _initSpec(name, cfg) {
  ...
  // fix height
  if (cfg.fix && spec.heightFix) {
    spec.height = spec.heightFix;
  }

  // full output path
  var _path = (cfg.locale && spec.localePath) ? spec.localePath.replace(':locale:', cfg.locale) : spec.path;
  spec.output = path.join(cfg.outputDir, _path.replace(':assets:', cfg.assetsDir));
