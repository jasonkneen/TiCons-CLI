...
module.exports = function config(opts, callback) {
  ...
  if (cfg.minDpi && !_.isNumber(cfg.minDpi) && constants.dpi[cfg.minDpi]) {
    cfg.minDpi = constants.dpi[cfg.minDpi];
  }
  if (cfg.maxDpi && !_.isNumber(cfg.maxDpi) && constants.dpi[cfg.maxDpi]) {
    cfg.maxDpi = constants.dpi[cfg.maxDpi];
  }

  var validDpi = _.values(constants.dpi);

  if (validDpi.indexOf(cfg.minDpi) === -1) {
    return callback('Invalid value for `min-dpi`.')
  }

  if (validDpi.indexOf(cfg.maxDpi) === -1) {
    return callback('Invalid value for `max-dpi`.')  
  }
